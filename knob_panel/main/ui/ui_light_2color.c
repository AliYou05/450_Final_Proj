...
extern void notify_brightness_change(uint8_t brightness);
        ...
        if (is_time_out(&time_500ms)) {
            if (LV_KEY_RIGHT == key) {
                if (light_set_conf.light_pwm < 100) {
                    ...
                    notify_brightness_change(light_set_conf.light_pwm); // Notify brightness change
    }
    ...
    if (is_time_out(&time_20ms)) {
        ...
        if ((light_set_conf.light_pwm ^ light_xor.light_pwm) || (light_set_conf.light_cck ^ light_xor.light_cck)) {
            ...
            default:
              ...
              notify_brightness_change(light_xor.light_pwm);
        }
    }
}
